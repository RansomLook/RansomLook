...
from ransomlook.default import DB_NOTES
...
def rconn() -> valkey.Valkey:
    return valkey.Valkey(unix_socket_path=get_socket_path("cache"), db=DB_NOTES)


def resolve_group(r: valkey.Valkey, raw: str) -> str | None:
    ...
    if mapped:
        return mapped.decode("utf-8")  # type: ignore[union-attr]
    return None


def save_note(r: valkey.Valkey, note: dict[str, Any]) -> None:
    note_id = note["id"]
    pipe = r.pipeline()
...
def append_version(r: valkey.Valkey, note_id: str, payload: dict[str, Any]) -> None:
    payload = {**payload, "ts": now_iso()}
    r.xadd(f"stream:note:{note_id}:versions", payload)


def audit(r: valkey.Valkey, action: str, **kw: object) -> None:
...
def ensure_repo() -> str:
    ...
    if not REPO_PATH.exists():
        ...
        logger.info("Updating repo in %s", REPO_PATH)
...
def run_import() -> None:
    ...
    if not r.set("lock:notes:import", "1", nx=True, ex=900):
        logger.info("Another import is running. Abort.")
        return
    ...
    try:
        ...
        logger.info("Import done. new=%s updated=%s skipped=%s", imported, updated, skipped)
    finally:
