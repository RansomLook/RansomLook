...
logger = get_logger("run_backend")


def check_running(name: str) -> bool:
    socket_path = get_socket_path(name)
    logger.debug(socket_path)
...
def shutdown_cache(storage_directory: Path | None = None) -> None:
    ...
    logger.info("Redis cache database shutdown.")
    ...
    while True:
        ...
        for db_name, running in backends.items():
            if not stop and not running:
                logger.info("Waiting on %s to start", db_name)
            if stop and running:
                logger.info("Waiting on %s to stop", db_name)
        time.sleep(1)
