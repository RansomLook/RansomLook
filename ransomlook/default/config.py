...
def get_config(config_type: str, entry: str, quiet: bool = False) -> Any:
    ...
    if config_type in configs:
        if entry in configs[config_type]:
            ...
            if not quiet:
                logger.warning("Unable to find %s in config file.", entry)
    else:
        if not quiet:
            logger.warning("No %s config file available.", config_type)
    if not quiet:
        logger.warning("Falling back on sample config, please initialize the %s config file.", config_type)
