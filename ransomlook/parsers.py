...
def cuba():
    stdlog('parser: ' + 'cuba')
    parser = '''
    grep '<p>' source/cuba-*.html --no-filename | cut -f2 -d'"' | cut -f4 -d/
    '''
    posts = runshellcmd(parser)
    if len(posts) == 1:
...
def alphav():
    ...
    parser = '''
    jq -r '.items[].title' source/alphv*.html | sed -e 's/^ *//g' -e 's/[[:space:]]*$//'
    '''
#    parser = '''
...
def nightsky():
    ...
    parser = '''
    egrep  'mdui-card-primary-title' --no-filename source/nightsky-*  |cut -d'>' -f3 |cut -f1 -d'<'
    '''
    posts = runshellcmd(parser)
    if len(posts) == 1:
...
def cooming():
    stdlog('parser: ' + 'Cooming')
    parser = '''
    grep 'title="' source/cooming-* | cut -f 14 -d'"'
    '''
    posts = runshellcmd(parser)
    if len(posts) == 1:
        errlog('Cooming: ' + 'parsing fail')
    for post in posts:
        appender(post, 'cooming')

def vicesociety():
    stdlog('parser: ' + 'ViceSociety')
    parser = '''
    grep 'color="#FFFFFF"' source/vicesociety-*.html  | cut -f7 -d'"' | cut -f3 -d'>'|cut -f1 -d'<'
    '''
    #grep '<tr><td valign="top"><br><font size="4" color="#FFFFFF"' source/vicesociety-*.html  | cut -f 6 -d '>' | cut -f1 -d'<'
    posts = runshellcmd(parser)
    if len(posts) == 1:
...
def pandora():
    ...
    parser = '''
    grep 'post-title gt-c-content-color-first' source/pandora-*.html  | cut -f2 -d'>' | cut -f1 -d'<'
    '''
    posts = runshellcmd(parser)
    if len(posts) == 1:
